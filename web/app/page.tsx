    ...
    <>
        ...
        <div className="mx-auto grid max-w-7xl gap-12 px-5 pb-20 pt-14 sm:px-8 lg:grid-cols-[1.05fr_0.95fr] lg:px-12 lg:pb-28 lg:pt-20">
          <div className="max-w-2xl">
            ...
            <div className="mt-9 flex flex-wrap gap-3">
              <Link className="inline-flex items-center gap-2 rounded-full bg-accent px-5 py-3 font-semibold text-[#031721] transition hover:bg-white focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-white" href="/explorar">Ver o percurso <ArrowRight aria-hidden="true" size={18} /></Link>
              <Link className="inline-flex items-center gap-2 rounded-full border border-white/25 px-5 py-3 font-semibold text-white transition hover:border-primary hover:bg-white/10 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-white" href="/glossario">Consultar termos <CircleHelp aria-hidden="true" size={18} /></Link>
            </div>
          </div>
      ...
      <section id="fluxo" className="mx-auto max-w-7xl px-5 py-18 sm:px-8 lg:px-12 lg:py-24">
        ...
        <div className="mt-10 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {steps.map(({ number, title, description, icon: Icon, tone }) => <Link href={`/${title.toLowerCase()}`} key={number} className="group rounded-2xl border border-border bg-card p-5 transition hover:-translate-y-1 hover:shadow-lg focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-primary"><article><div className="flex items-start justify-between"><span className={`grid size-11 place-items-center rounded-xl text-[#031721] ${tone}`}><Icon aria-hidden="true" size={20} /></span><span className="font-mono text-sm text-muted-foreground">{number}</span></div><h3 className="mt-7 text-xl font-semibold tracking-tight">{title}</h3><p className="mt-2 leading-6 text-muted-foreground">{description}</p><span className="mt-6 inline-flex items-center gap-1 text-sm font-semibold text-primary">Abrir etapa <ArrowRight aria-hidden="true" size={15} /></span></article></Link>)}
        </div>
      </section>
